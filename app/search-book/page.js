...
import engineeringclgdata from "./engineeringclgdata.json";
...
const Search = () => {
  ...
  // Find courses for selected university
  const selectedUniversity = engineeringclgdata.find(
    (item) => item.universityName === university
  );
  ...
  const handleSubmit = async (event) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      router.push(`/branch-year?university=${university}`);
    }, 1500);
  };
        ...
        <div className="row justify-content-center">
          <div className="col-12">
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="search-form">
                  <div className="form-group">
                    <select
                      ...
                      required
                    >
                      {/* Iterating through college json data */}
                      <option value="">🎓 Choose Your University</option>
                      {engineeringclgdata.map((item, index) => (
                  ...
                  {/*for now removed dependency on courses as API only consist of college names*/}

                  {/* Course Dropdown (optional) */}
                  {courses.length > 0 && (
                    <div className="form-group">
                      <select
                        className={`form-control ${course ? "success" : ""}`}
                        value={course}
              ...
              <div
                className={`stats-row ${
                  animationStep >= 3 ? "fade-in-delay-2" : ""
                }`}
              >
