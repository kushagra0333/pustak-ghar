  ...
  <div className="max-w-7xl mx-auto px-6">
    ...
    <div className="grid lg:grid-cols-2 gap-8">
      {features.map((feature, index) => (
        <div
          key={index}
          className="group relative bg-white border-2 border-amber-300 rounded-3xl p-8 flex flex-col h-full transition-all duration-500 hover:shadow-xl hover:scale-[1.02] hover:border-rose-400"
        >
          <div className="flex items-start space-x-6 mb-4">
            <div
              className={`w-16 h-16 bg-gradient-to-r ${feature.color} rounded-2xl flex items-center justify-center text-white flex-shrink-0 transform transition-transform duration-300 group-hover:scale-110 shadow-lg`}
            >
              {feature.icon}
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-bold text-amber-900 mb-2 group-hover:text-rose-700 transition-colors">
                {feature.title}
              </h3>
              <p className="text-amber-700 leading-relaxed group-hover:text-amber-800 transition-colors">
                {feature.description}
              </p>
            </div>
          </div>
          <div className="mt-auto pt-2">
            <span className="inline-flex items-center text-rose-700 font-semibold group-hover:text-pink-800 transition-all">
              Explore More
